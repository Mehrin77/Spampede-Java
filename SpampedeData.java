...
class SpampedeData {
	 ...
	 */
	 //public void moveSnakeForward(BoardCell cell) {
		/*if (cell.isSpam()) {
	        // If the next cell is spam, eat the spam and grow the snake
	        moveSnakeForward(cell);
	    } else {
	        // If the next cell is not spam, move the snake forward
	        moveSnakeForwardToEmptyCell(cell); */
	    //}
	

	// Helper method to move the snake to an empty cell
	//private void moveSnakeForwardToEmptyCell(BoardCell cell) {
	    // Remove the first cell (tail) from the snake
    public void moveSnakeForward(BoardCell cell) {
		    ...
		    else if( (cell.isWall()) || (cell.isBody()) ) {
				setGameOver ();
	...
	private void BoardCell(BoardCell cell) {
 ...
 // TODO: Implement getNorthNeighbor(BoardCell)
  ...
  public BoardCell getNextCellFromBFS() {
		    ...
		    while(!cellsToSearch.isEmpty()) {
		    	BoardCell holder = cellsToSearch.remove();
		    	BoardCell[] neighbor = this.getNeighbors(holder);
		    	if (holder.isSpam()) {
		    		return getFirstCellInPath(holder);
		    	}
		   
		    	for (BoardCell cell : neighbor) {
		    		if (cell.isOpen() && !cell.inSearchListAlready()) {
  ...
  private BoardCell getFirstCellInPath(BoardCell start) {
		    ...
		    // follow the traceback pointers until reaching the snake head
		    while (currentCell.getParent() != this.getSnakeHead()) {
		        currentCell = currentCell.getParent();
		    }

		    // the next cell to move the snake head to is the neighbor of the snake head
		    return currentCell;
		}
   ...
   */
	public void reverseSnake() {
	    ...
	    LinkedList<BoardCell> helper = new LinkedList<BoardCell>(this.snakeCells);
	    Collections.reverse(helper);
	    this.snakeCells = helper;

	    // Step 3: relabel the head
	    head = getSnakeHead();
	    head.becomeHead();
	    BoardCell helperHead = this.getSnakeHead();
	    BoardCell helperNeck = this.getSnakeNeck();

	    if (helperHead.getColumn() == helperNeck.getColumn()) {
	        if (helperHead.getRow() > helperNeck.getRow()) {
