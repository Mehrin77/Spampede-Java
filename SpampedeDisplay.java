...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		// Draw the board
		for (int x = 0; x < this.theData.getNumColumns()  ; x ++) {
			for (int y = 0; y < this.theData.getNumRows(); y ++) {
				this.drawSquare(x * Preferences.CELL_SIZE,y * Preferences.CELL_SIZE,this.theData.getCellColor(y, x));
			}
		}
	...
	private void drawSquare(int x, int y, Color cellColor) {
		int pad_x = (this.width - (Preferences.NUM_CELLS_WIDE * Preferences.CELL_SIZE))/2;
		int pad_y = Preferences.TITLE_Y + Preferences.CELL_SIZE;
		this.theScreen.setColor(cellColor);
		this.theScreen.fillRect(x + pad_x, y + pad_y, Preferences.CELL_SIZE, Preferences.CELL_SIZE);
	}
